...
| 2 — Secure Webhook and Integration Foundation | In progress | Day 156 added fail-closed integration configuration: a public HTTPS origin must match an exact host allowlist; enabled settings require separate inbound/outbound secrets and a clean workflow path; timeouts, body/response limits, and signature age have safe bounds; unknown environment labels are rejected. Day 157 added the versioned `abap.webhook.v1` schema and HMAC-SHA256 signing contract. It rejects changed signed data, invalid signatures, stale/future timestamps, oversized requests, wrong content types, malformed JSON, unsafe schema fields, and noncanonical event IDs. Its focused suite passed (6 tests). Day 156 remains the latest confirmed full-suite result: 651 tests ran with 4 optional PostgreSQL tests skipped because `ABAP_TEST_DATABASE_URL` was not configured. No webhook is sent or received yet; replay persistence, delivery history, private n8n transport, connection-time DNS/redirect checks, and a final full-suite run remain. The Milestone 2 exit gate is not met. |
...
Continue Milestone 2 on Day 158 with persistent event-ID replay protection and
safe webhook-delivery state. Keep the worker boundaries intact; do not begin
n8n integration until the Milestone 2 signed, replay-safe webhook exit gate
passes.
