...
| 2 — Secure Webhook and Integration Foundation | In progress | Day 156 added fail-closed integration configuration: a public HTTPS origin must match an exact host allowlist; enabled settings require separate inbound/outbound secrets and a clean workflow path; timeouts, body/response limits, and signature age have safe bounds; unknown environment labels are rejected. Day 157 added the versioned `abap.webhook.v1` schema and HMAC-SHA256 signing contract. Day 158 added durable event-ID replay protection and payload-free delivery records. Day 159 added signed outbound HTTPS delivery with connection-time public DNS checks, TLS hostname validation, redirect rejection, response limits, bounded immediate retries, and safe status updates. Day 160 added a fail-closed inbound callback route with bounded streaming reads, HMAC/time-window verification, atomic replay claiming, and generic safe responses; it also added an administrator-only delivery history page that shows metadata only. Day 161 added a separate retry worker that leases due outbound records, reconstructs only allowlisted workflow-completion messages from durable execution state, retries abandoned leases after restart, and terminally records any unreconstructable record without sending it. Day 162 added an atomic signed result handler that accepts only a fixed completed/failed outcome for running schedule-triggered executions; it finishes their task snapshots with a fixed safe summary and leaves manual runs unchanged. Focused result-callback, webhook, and workflow tests passed (37 total). Day 161's focused set of 50 tests remains confirmed. A broader web/workflow test process ended after progress output without a final exit record, so it is not claimed as passing. Day 156 remains the latest confirmed full-suite result: 651 tests ran with 4 optional PostgreSQL tests skipped because `ABAP_TEST_DATABASE_URL` was not configured. A deterministic authenticated round-trip test and a confirmed final full-suite run remain. The Milestone 2 exit gate is not met. |
...
Continue Milestone 2 on Day 163 with a deterministic local authenticated
round-trip test: one signed outbound request, one signed callback, and one
tracked outcome despite a duplicate callback. Do not begin live n8n
integration until the Milestone 2 signed, replay-safe webhook exit gate passes.
